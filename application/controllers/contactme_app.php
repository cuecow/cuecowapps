<?php
class contactme_app extends CI_Controller {
    ...
    public function index()
    {
       ...
       // echo "<pre>";print_r($comparr);echo "</pre>";
        ...
        $data['appid'] = $fbappid;
         //echo "<pre>";print_r($tempid);echo "</pre>";
        if(isset($_POST['emialid']))
                { 
            
                    $this->loadtemplates->canvas_save_email($tempid,$pid,$fbappid);
                    redirect("contactme_app?signed_request=".$_REQUEST['signed_request']."&appid=".$_REQUEST['appid']."&issubmit=1");
                }
            else 
                {
                 if($becomefan=="Yes")
